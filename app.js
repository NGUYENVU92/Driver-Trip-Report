...
const CLIENT_ID = '851875117831-l135siesbo6sdeo3i7ttkhm8d2v0ldko.apps.googleusercontent.com';
	...
	if (gapiInited && gisInited) document.getElementById('exportBtn').disabled = false;
	}

	async function uploadToDrive(fileBlob, filename) {
	const token = gapi.client.getToken();
	if (!token) {
    tokenClient.callback = async () => await uploadToDrive(fileBlob, filename);
	...
	alert('File saved to Google Drive! ID: ' + data.id);
	}

// After creating the CSV blob
